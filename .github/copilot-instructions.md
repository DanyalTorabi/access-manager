...
- **Git (AI default):** AI agents may automatically run `git commit`, `git push`, and `gh pr create` as needed. Ensure commit messages are detailed (subject + body format), PR descriptions follow `.github/pull_request_template.md` (Summary, Ticket, Checklist), and PR titles start with `[TXX]` where TXX is the ticket number (e.g. `[T65] split long api files`).
- **Use `gh` for automation:** Prefer the `gh` CLI over raw HTTP calls for GitHub operations; when automation is not permitted, supply copy-paste snippets for maintainers.
