...
- [ ] Refactored e2e helpers (`mustPATCH`, `mustDELETE`, list decoder, error helpers, seed helpers).
- [ ] Full CRUD lifecycle tests for every entity type (domains, users, groups, resources, access types, permissions).
- [ ] Authz challenge scenarios (direct, group-inherited, no permission, multiple masks, revoke-recheck).
- [ ] Pagination journey (page-through, past-total, default params).
- [ ] Error path journeys (invalid UUIDs, missing fields, duplicates, referential integrity, bad pagination).
- [ ] Auth journey (missing/wrong/valid bearer token; health bypass).
- [ ] Deprecation notice on `test/e2e/bash/run.sh`.
