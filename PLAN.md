...
4. **Org Backend Engineering curriculum (applicable themes only):** config file + env, graceful HTTP shutdown, `go test -race`, `go vet` + **golangci-lint**, Docker (scratch-style image + compose for integration tests), CI on Ubuntu with integration + **GHCR** publish on merge to `main`, Prometheus/Grafana when ready, Kubernetes after Docker—see [`docs/backend-curriculum.md`](docs/backend-curriculum.md). **Work and status** live in [GitHub Issues](https://github.com/DanyalTorabi/access-manager/issues) only. Event streaming (Kafka/Redpanda), Protobuf, and Cassandra are **out of scope** for this SQL/HTTP service unless requirements change.
