...
- **T04 / #15:** Materialized `user_resource_masks` table for the authz hot path (SQLite). `EffectiveMask`, `UserAuthzResourcesList`, and `ResourceAuthzUsersList` now read from a precomputed O(1) index lookup instead of multi-JOIN query-time aggregations. All 6 authz mutation methods (`Grant/Revoke UserPermission`, `Grant/Revoke GroupPermission`, `Add/RemoveUserFromGroup`) maintain the cache transactionally. `ReconcileUserResourceMasks` rebuilds the table from source tables at server startup. Property tests verify materialized result == ground-truth path after every mutation.

- **T05 / #16:** Go micro-benchmarks for the SQLite authz store layer (`BenchmarkEffectiveMask`, `BenchmarkResourceAuthzUsersList`, `BenchmarkUserAuthzResourcesList`, `BenchmarkResourceAuthzGroupsList_PageNearParamCap`, `BenchmarkGroupSetParent_DeepChain`) and `access.CombineMasks`. Covers the T44/CML3/CML9 deferred benchmark requests. k6 HTTP load script at `test/load/authz.js` with ramp-up stages (0→10→50 VUs), p99 < 50 ms and error-rate < 1% thresholds, and saturation/soak variants. Shared `buildInQueryAndArgs` helper extracted from the duplicated "select IDs then IN(…) aggregate masks" pattern across four SQLite store listing methods, deferred from PR #75 review.
...
- **T22 / #33:** `CORS_ALLOWED_ORIGINS=none` sentinel — setting the env var (or `cors_allowed_origins: "none"` in YAML) to the case-insensitive string `none` (whitespace-trimmed) resolves to an empty origin slice, disabling all in-process CORS response headers. Use when a reverse proxy manages CORS centrally. `compose.prod.yml` (standalone production example) and `docs/environments.md` (tier matrix, secrets mapping, rollback guide) added.
...
- **T51 / #77:** SQLite migration `000003_composite_fk_cross_domain` — composite foreign keys on `group_members`, `user_permissions`, and `group_permissions` so each row's `domain_id` is enforced to match the referenced user/group/permission's `domain_id`. The migration aborts loudly via a temporary `RAISE(ABORT, ...)` trigger if existing data contains cross-domain rows. Authz listings (`UserAuthzResourcesList`, `GroupAuthzResourcesList`, `ResourceAuthzUsersList`, `ResourceAuthzGroupsList` and `PermissionMasksForUserResource`) now rely solely on the schema invariant for cross-domain isolation; the previously layered defensive Go-side `domain_id` filters were dropped in lockstep. A companion `000003_composite_fk_cross_domain.down.sql` ships for manual rollback (operator-run; the in-tree migrator only applies `.up.sql`).
- **T50 / #74:** `store_negative_mask_observed_total` Prometheus counter, bumped whenever the SQLite store reads a negative `int64` access mask (via `maskFromSQL`). Wired in `cmd/server` through the new `(*sqlite.Store).SetNegativeMaskHook` method on the constructed store. Operators can alert on a non-zero value to detect legacy or out-of-band data. Existing `slog.Warn` is retained.
