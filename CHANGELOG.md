...
- **T51 / #77:** SQLite migration `000003_composite_fk_cross_domain` — composite foreign keys on `group_members`, `user_permissions`, and `group_permissions` so each row's `domain_id` is enforced to match the referenced user/group/permission's `domain_id`. The migration aborts loudly via a CHECK trip if existing data contains cross-domain rows. Authz listings keep their defensive `g.domain_id`/`u.domain_id`/`p.domain_id` filters; dropping them is deferred to a follow-up so all queries change in lockstep.
