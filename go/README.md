...
| Concurrent (race) | `make test-concurrent` — runs `TestConcurrent_*` with `-race -count=3` to flush flaky races (T63). Skipped by `go test -short`; included in `make test` by default. |
