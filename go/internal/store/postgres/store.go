...
// rebind replaces each `?` placeholder with `$N` (1-based) for PostgreSQL.
...
// maskToSQL converts a uint64 mask into a signed int64 for PostgreSQL BIGINT
// storage. Returns an error if the value cannot be represented in signed
// 64-bit (i.e. uses bit 63).
...
// maskFromSQL converts an int64 value read from the DB into uint64. If a
// negative value is encountered, it logs a warning and returns 0.
...
func (s *Store) DomainCreate(ctx context.Context, d *store.Domain) error {
	_, err := s.db.ExecContext(ctx, rebind(`INSERT INTO domains (id, title) VALUES (?, ?)`), d.ID, d.Title)
	return wrapConstraintError(err)
}

func (s *Store) DomainGet(ctx context.Context, id string) (*store.Domain, error) {
	row := s.db.QueryRowContext(ctx, rebind(`SELECT id, title FROM domains WHERE id = ?`), id)
...
func (s *Store) DomainList(ctx context.Context, opts store.ListOpts) ([]store.Domain, int64, error) {
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(*) FROM domains`+where), args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT id, title FROM domains`+where+orderByClause(opts.Sort, opts.Order, domainSortColumns, "title")+` LIMIT ? OFFSET ?`), // #nosec G202: ORDER BY column from allow-list
...
func (s *Store) DomainDelete(ctx context.Context, id string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM domains WHERE id = ?`), id)
	if err != nil {
		return wrapConstraintError(err)
	}
...
func (s *Store) DomainPatch(ctx context.Context, id string, title *string) (*store.Domain, error) {
	...
	if _, err := s.db.ExecContext(ctx, rebind(`UPDATE domains SET title = ? WHERE id = ?`), *title, id); err != nil {
...
func (s *Store) UserCreate(ctx context.Context, u *store.User) error {
	_, err := s.db.ExecContext(ctx, rebind(`INSERT INTO users (id, domain_id, title) VALUES (?, ?, ?)`),
		u.ID, u.DomainID, u.Title)
	return wrapConstraintError(err)
}

func (s *Store) UserGet(ctx context.Context, domainID, id string) (*store.User, error) {
	row := s.db.QueryRowContext(ctx, rebind(`SELECT id, domain_id, title FROM users WHERE id = ? AND domain_id = ?`), id, domainID)
	var out store.User
...
func (s *Store) UserList(ctx context.Context, domainID string, opts store.ListOpts) ([]store.User, int64, error) {
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(*) FROM users `+where), args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT id, domain_id, title FROM users `+where+orderByClause(opts.Sort, opts.Order, userSortColumns, "title")+` LIMIT ? OFFSET ?`), // #nosec G202
		append(args, opts.Limit, opts.Offset)...)
	if err != nil {
		return nil, 0, err
	}
...
func (s *Store) UserDelete(ctx context.Context, domainID, id string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM users WHERE id = ? AND domain_id = ?`), id, domainID)
	if err != nil {
		return wrapConstraintError(err)
	}
	n, err := res.RowsAffected()
...
func (s *Store) UserPatch(ctx context.Context, domainID, id string, title *string) (*store.User, error) {
	...
	if _, err := s.db.ExecContext(ctx, rebind(`UPDATE users SET title = ? WHERE id = ? AND domain_id = ?`), *title, id, domainID); err != nil {
		return nil, wrapConstraintError(err)
	}
...
func (s *Store) GroupCreate(ctx context.Context, g *store.Group) error {
	...
	_, err := s.db.ExecContext(ctx, rebind(`INSERT INTO groups (id, domain_id, title, parent_group_id) VALUES (?, ?, ?, ?)`),
		g.ID, g.DomainID, g.Title, parent)
	return wrapConstraintError(err)
}

func (s *Store) GroupGet(ctx context.Context, domainID, id string) (*store.Group, error) {
	row := s.db.QueryRowContext(ctx, rebind(`SELECT id, domain_id, title, parent_group_id FROM groups WHERE id = ? AND domain_id = ?`), id, domainID)
	var out store.Group
...
func (s *Store) GroupList(ctx context.Context, domainID string, opts store.GroupListOpts) ([]store.Group, int64, error) {
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(*) FROM groups `+where), args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT id, domain_id, title, parent_group_id FROM groups `+where+orderByClause(opts.Sort, opts.Order, groupSortColumns, "title")+` LIMIT ? OFFSET ?`), // #nosec G202
		append(args, opts.Limit, opts.Offset)...)
	if err != nil {
		return nil, 0, err
	}
...
func groupGetTx(ctx context.Context, tx *sql.Tx, domainID, id string) (*store.Group, error) {
	row := tx.QueryRowContext(ctx, rebind(`SELECT id, domain_id, title, parent_group_id FROM groups WHERE id = ? AND domain_id = ?`), id, domainID)
	var out store.Group
	var parent sql.NullString
	if err := row.Scan(&out.ID, &out.DomainID, &out.Title, &parent); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func groupSetParentTx(ctx context.Context, tx *sql.Tx, domainID, groupID string, parentID *string) error {
	...
	_, err := tx.ExecContext(ctx, rebind(`UPDATE groups SET parent_group_id = ? WHERE id = ? AND domain_id = ?`), parent, groupID, domainID)
	return wrapConstraintError(err)
}
...
func (s *Store) GroupDelete(ctx context.Context, domainID, id string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM groups WHERE id = ? AND domain_id = ?`), id, domainID)
	if err != nil {
		return wrapConstraintError(err)
	}
	n, err := res.RowsAffected()
...
func (s *Store) GroupPatch(ctx context.Context, domainID, groupID string, p store.GroupPatchParams) (*store.Group, error) {
	...
	if p.Title != nil {
		if _, err := tx.ExecContext(ctx, rebind(`UPDATE groups SET title = ? WHERE id = ? AND domain_id = ?`), *p.Title, groupID, domainID); err != nil {
			return nil, wrapConstraintError(err)
		}
	}
	if p.UpdateParent {
...
func (s *Store) ResourceCreate(ctx context.Context, r *store.Resource) error {
	_, err := s.db.ExecContext(ctx, rebind(`INSERT INTO resources (id, domain_id, title) VALUES (?, ?, ?)`),
		r.ID, r.DomainID, r.Title)
	return wrapConstraintError(err)
}

func (s *Store) ResourceGet(ctx context.Context, domainID, id string) (*store.Resource, error) {
	row := s.db.QueryRowContext(ctx, rebind(`SELECT id, domain_id, title FROM resources WHERE id = ? AND domain_id = ?`), id, domainID)
...
func (s *Store) ResourceList(ctx context.Context, domainID string, opts store.ListOpts) ([]store.Resource, int64, error) {
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(*) FROM resources `+where), args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT id, domain_id, title FROM resources `+where+orderByClause(opts.Sort, opts.Order, resourceSortColumns, "title")+` LIMIT ? OFFSET ?`), // #nosec G202
		append(args, opts.Limit, opts.Offset)...)
	if err != nil {
		return nil, 0, err
	}
...
func (s *Store) ResourceDelete(ctx context.Context, domainID, id string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM resources WHERE id = ? AND domain_id = ?`), id, domainID)
	if err != nil {
		return wrapConstraintError(err)
	}
	n, err := res.RowsAffected()
...
func (s *Store) ResourcePatch(ctx context.Context, domainID, id string, title *string) (*store.Resource, error) {
	...
	if _, err := s.db.ExecContext(ctx, rebind(`UPDATE resources SET title = ? WHERE id = ? AND domain_id = ?`), *title, id, domainID); err != nil {
		return nil, wrapConstraintError(err)
	}
...
func (s *Store) AccessTypeCreate(ctx context.Context, a *store.AccessType) error {
	...
	_, err = s.db.ExecContext(ctx, rebind(`INSERT INTO access_types (id, domain_id, title, bit) VALUES (?, ?, ?, ?)`),
		a.ID, a.DomainID, a.Title, bitVal)
	return wrapConstraintError(err)
}

func (s *Store) AccessTypeGet(ctx context.Context, domainID, id string) (*store.AccessType, error) {
	row := s.db.QueryRowContext(ctx, rebind(`SELECT id, domain_id, title, bit FROM access_types WHERE id = ? AND domain_id = ?`), id, domainID)
...
func (s *Store) AccessTypeList(ctx context.Context, domainID string, opts store.ListOpts) ([]store.AccessType, int64, error) {
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(*) FROM access_types `+where), args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT id, domain_id, title, bit FROM access_types `+where+orderByClause(opts.Sort, opts.Order, accessTypeSortColumns, "title")+` LIMIT ? OFFSET ?`), // #nosec G202
		append(args, opts.Limit, opts.Offset)...)
	if err != nil {
		return nil, 0, err
	}
...
func (s *Store) AccessTypeDelete(ctx context.Context, domainID, id string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM access_types WHERE id = ? AND domain_id = ?`), id, domainID)
	if err != nil {
		return wrapConstraintError(err)
	}
	n, err := res.RowsAffected()
...
func (s *Store) AccessTypePatch(ctx context.Context, domainID, id string, p store.AccessTypePatchParams) (*store.AccessType, error) {
	...
	defer func() { _ = tx.Rollback() }()
	row := tx.QueryRowContext(ctx, rebind(`SELECT title, bit FROM access_types WHERE id = ? AND domain_id = ?`), id, domainID)
	var curTitle string
	var curBit int64
	...
	if _, err := tx.ExecContext(ctx, rebind(`UPDATE access_types SET title = ?, bit = ? WHERE id = ? AND domain_id = ?`),
...
func (s *Store) PermissionCreate(ctx context.Context, p *store.Permission) error {
	...
	_, err = s.db.ExecContext(ctx, rebind(`INSERT INTO permissions (id, domain_id, title, resource_id, access_mask) VALUES (?, ?, ?, ?, ?)`),
...
func (s *Store) PermissionGet(ctx context.Context, domainID, id string) (*store.Permission, error) {
	row := s.db.QueryRowContext(ctx, rebind(`SELECT id, domain_id, title, resource_id, access_mask FROM permissions WHERE id = ? AND domain_id = ?`), id, domainID)
...
func (s *Store) PermissionList(ctx context.Context, domainID string, opts store.PermissionListOpts) ([]store.Permission, int64, error) {
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(*) FROM permissions `+where), args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT id, domain_id, title, resource_id, access_mask FROM permissions `+where+orderByClause(opts.Sort, opts.Order, permissionSortColumns, "title")+` LIMIT ? OFFSET ?`), // #nosec G202
		append(args, opts.Limit, opts.Offset)...)
	if err != nil {
		return nil, 0, err
	}
...
func (s *Store) PermissionDelete(ctx context.Context, domainID, id string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM permissions WHERE id = ? AND domain_id = ?`), id, domainID)
	if err != nil {
		return wrapConstraintError(err)
	}
	n, err := res.RowsAffected()
...
func (s *Store) PermissionPatch(ctx context.Context, domainID, id string, p store.PermissionPatchParams) (*store.Permission, error) {
	...
	defer func() { _ = tx.Rollback() }()
	row := tx.QueryRowContext(ctx, rebind(`SELECT title, resource_id, access_mask FROM permissions WHERE id = ? AND domain_id = ?`), id, domainID)
	...
	if p.ResourceID != nil {
		var exists int
		if err := tx.QueryRowContext(ctx, rebind(`SELECT 1 FROM resources WHERE id = ? AND domain_id = ?`), *p.ResourceID, domainID).Scan(&exists); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return nil, store.ErrNotFound
			}
			return nil, err
	...
	if _, err := tx.ExecContext(ctx, rebind(`UPDATE permissions SET title = ?, resource_id = ?, access_mask = ? WHERE id = ? AND domain_id = ?`),
...
func (s *Store) AddUserToGroup(ctx context.Context, domainID, userID, groupID string) error {
	_, err := s.db.ExecContext(ctx, rebind(`INSERT INTO group_members (domain_id, user_id, group_id) VALUES (?, ?, ?)`),
		domainID, userID, groupID)
	return wrapConstraintError(err)
}

func (s *Store) RemoveUserFromGroup(ctx context.Context, domainID, userID, groupID string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM group_members WHERE domain_id = ? AND user_id = ? AND group_id = ?`),
		domainID, userID, groupID)
	if err != nil {
		return wrapConstraintError(err)
	}
...
func (s *Store) GrantUserPermission(ctx context.Context, domainID, userID, permissionID string) error {
	_, err := s.db.ExecContext(ctx, rebind(`INSERT INTO user_permissions (domain_id, user_id, permission_id) VALUES (?, ?, ?)`),
...
func (s *Store) RevokeUserPermission(ctx context.Context, domainID, userID, permissionID string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM user_permissions WHERE domain_id = ? AND user_id = ? AND permission_id = ?`),
		domainID, userID, permissionID)
	if err != nil {
		return wrapConstraintError(err)
	}
...
func (s *Store) GrantGroupPermission(ctx context.Context, domainID, groupID, permissionID string) error {
	_, err := s.db.ExecContext(ctx, rebind(`INSERT INTO group_permissions (domain_id, group_id, permission_id) VALUES (?, ?, ?)`),
...
func (s *Store) RevokeGroupPermission(ctx context.Context, domainID, groupID, permissionID string) error {
	res, err := s.db.ExecContext(ctx, rebind(`DELETE FROM group_permissions WHERE domain_id = ? AND group_id = ? AND permission_id = ?`),
		domainID, groupID, permissionID)
	if err != nil {
		return wrapConstraintError(err)
	}
...
func buildUserAuthzMaskQueryAndArgs(domainID string, resourceIDs []string, predicateArgs []any) (string, []any, error) {
	...
	query := rebind(`SELECT p.resource_id, p.access_mask FROM permissions p WHERE p.domain_id = ? AND p.resource_id IN (` + placeholders + `) AND p.access_mask > 0` + userEffectivePermissionPredicateSQL) // #nosec G202
...
func (s *Store) UserAuthzResourcesList(ctx context.Context, domainID, userID string, opts store.ListOpts) ([]store.UserAuthzResource, int64, error) {
	...
	var exists int
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT 1 FROM domains WHERE id = ?`), domainID).Scan(&exists); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, 0, store.ErrNotFound
		}
		return nil, 0, err
	}
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT 1 FROM users WHERE id = ? AND domain_id = ?`), userID, domainID).Scan(&exists); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, 0, store.ErrNotFound
		}
		return nil, 0, err
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(DISTINCT p.resource_id) `+userAuthzResourcesBaseSQL), countArgs...).Scan(&total); err != nil {
		return nil, 0, err
	}
	...
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT DISTINCT p.resource_id `+userAuthzResourcesBaseSQL+` ORDER BY p.resource_id ASC LIMIT ? OFFSET ?`),
		listArgs...,
	)
	if err != nil {
		return nil, 0, err
...
func (s *Store) GroupAuthzResourcesList(ctx context.Context, domainID, groupID string, opts store.ListOpts) ([]store.GroupAuthzResource, int64, error) {
	...
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT 1 FROM groups WHERE id = ? AND domain_id = ?`), groupID, domainID).Scan(&exists); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, 0, store.ErrNotFound
		}
		return nil, 0, err
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(DISTINCT p.resource_id) `+groupAuthzResourcesBaseSQL), baseArgs...).Scan(&total); err != nil {
		return nil, 0, err
	}

	listArgs := append(append([]any{}, baseArgs...), opts.Limit, opts.Offset)
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT DISTINCT p.resource_id `+groupAuthzResourcesBaseSQL+` ORDER BY p.resource_id ASC LIMIT ? OFFSET ?`), // #nosec G202
		listArgs...,
	)
	if err != nil {
		return nil, 0, err
	...
	maskSQL := rebind(`SELECT p.resource_id, p.access_mask FROM permissions p ` + // #nosec G202
		`INNER JOIN group_permissions gp ON gp.permission_id = p.id ` +
...
func (s *Store) ResourceAuthzGroupsList(ctx context.Context, domainID, resourceID string, opts store.ListOpts) ([]store.ResourceAuthzGroup, int64, error) {
	...
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT 1 FROM resources WHERE id = ? AND domain_id = ?`), resourceID, domainID).Scan(&exists); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, 0, store.ErrNotFound
		}
		return nil, 0, err
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(DISTINCT gp.group_id) `+resourceAuthzGroupsBaseSQL), baseArgs...).Scan(&total); err != nil {
		return nil, 0, err
	}

	listArgs := append(append([]any{}, baseArgs...), opts.Limit, opts.Offset)
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT DISTINCT gp.group_id `+resourceAuthzGroupsBaseSQL+` ORDER BY gp.group_id ASC LIMIT ? OFFSET ?`), // #nosec G202
		listArgs...,
	)
	if err != nil {
		return nil, 0, err
	...
	maskSQL := rebind(`SELECT gp.group_id, p.access_mask FROM permissions p ` + // #nosec G202
		`INNER JOIN group_permissions gp ON gp.permission_id = p.id ` +
		`INNER JOIN groups g ON g.id = gp.group_id ` +
		`WHERE p.domain_id = ? AND p.resource_id = ? AND p.access_mask > 0 ` +
...
func (s *Store) ResourceAuthzUsersList(ctx context.Context, domainID, resourceID string, opts store.ListOpts) ([]store.ResourceAuthzUser, int64, error) {
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, rebind(`SELECT COUNT(*) `+resourceAuthzUsersBaseSQL), baseArgs...).Scan(&total); err != nil {
		return nil, 0, err
	}

	listArgs := append(append([]any{}, baseArgs...), opts.Limit, opts.Offset)
	rows, err := s.db.QueryContext(ctx,
		rebind(`SELECT u.id `+resourceAuthzUsersBaseSQL+` ORDER BY u.id ASC LIMIT ? OFFSET ?`),
		listArgs...,
	)
	if err != nil {
		return nil, 0, err
	...
	directSQL := rebind(`SELECT up.user_id, p.access_mask FROM user_permissions up ` + // #nosec G202
	...
	indirectSQL := rebind(`SELECT gm.user_id, p.access_mask FROM group_members gm ` + // #nosec G202
...
func (s *Store) PermissionMasksForUserResource(ctx context.Context, domainID, userID, resourceID string) ([]uint64, error) {
	...
	rows, err := s.db.QueryContext(ctx, rebind(effectiveMaskSQL), args...)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
